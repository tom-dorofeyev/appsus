...
function resetBooks(books) {
  return books.items.map(book => {
    return {
      ...
      listPrice: book.saleInfo.saleability === "FREE" ? {amount: "FREE"} : book.saleInfo.listPrice
    }
  })
}
...
function googleBooksQuery(books) {
  var books = storageService.load(GOOGLE_BOOKS_KEY);
  if (books) {
    getGoogleBooks().then(res => {
      books = resetBooks(res);
      storageService.store(GOOGLE_BOOKS_KEY, books)
    })
  }
  books = resetBooks(books);
  googleBooksDB = books;
  return Promise.resolve(books);
}

function getGoogleBooks(filterWord) {
  return fetch(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${filterWord}`)
    .then(res => res.json())
    .then(res => {
      googleBooksDB = resetBooks(res)
      return res
    })
}
...
function addToDB(book) {
  let isBookInDB = booksDB.some(DBbook => book.id === DBbook.id);
  if (isBookInDB) return
  booksDB.unshift(book);
}

function getGoogleBookById(bookId) {
  const book = googleBooksDB.find(book => book.id === bookId);
